...
export async function POST(req: Request) {
  try {
    ...
    if (!leagueSlug || !matchweekLabel || !Array.isArray(results) || results.length === 0) {
...
updates.push(
  supabase
    ...
    for (const pick of refreshedPicks || []) {
      ...
      totalsByEntry[key] = round2((totalsByEntry[key] || 0) + Number(pick.points || 0));
    }
